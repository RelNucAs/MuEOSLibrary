...
void make_complete_table() {
	...
	std::vector<std::array<double,9>> eos_out;
        ...
        string table_filename = abs_path + "eos_table/" + sp + "/eos_" + sp + "_primitive"+res+".txt";
	...
	Iout << std::scientific << setprecision(8);
        ...
        for (int i=0;i<n1;i++) n_array.push_back(log_nmin + static_cast<double>(i) * (log_nmax-log_nmin) / static_cast<double>(n1));
        for (int i=0;i<n2;i++) t_array.push_back(log_tmin + static_cast<double>(i) * (log_tmax-log_tmin) / static_cast<double>(n2));
	...
	for (int i=0;i<n1;i++) {
                for (int j=0;j<n2;j++) {
                        Iout << eos_out[n2*i+j][2] + eos_out[n2*i+j][3] << " ";
                }
                Iout << "\n";
        }
	...
	for (int i=0;i<n1;i++) {
                for (int j=0;j<n2;j++) {
                        Iout << eos_out[n2*i+j][4] + eos_out[n2*i+j][5] << " ";
                }
                Iout << "\n";
        }
	...
	for (int i=0;i<n1;i++) {
                for (int j=0;j<n2;j++) {
                        Iout << eos_out[n2*i+j][6] + eos_out[n2*i+j][7] << " ";
                }
                Iout << "\n";
        }
