...
eta_file = table_folder + sp + '/eos_'+sp+'_leo.txt'
ne = np.loadtxt(eta_file,skiprows=0,max_rows=1,dtype=int)
nt  = np.loadtxt(eta_file,skiprows=1,max_rows=1,dtype=int)
ne_array = 10.**(np.loadtxt(eta_file,skiprows=2,max_rows=1,dtype=float))
t_array  = 10.**(np.loadtxt(eta_file,skiprows=3,max_rows=1,dtype=float))
eta_leo = np.loadtxt(eta_file,skiprows=4,max_rows=n1,unpack=True,dtype=float)

eta_file = table_folder + sp +'/eos_'+sp+'_v4.txt'
...
plt.savefig(plot_folder + sp + "/compare_eta_"+sp+"_v4.png",dpi=300,bbox_inches='tight')
...
c1 = axs[0].contourf(X,Y,abs(antieta_leo),norm=colors.LogNorm(vmin=np.amin(abs(antieta_leo)),vmax=np.amax(abs(antieta_leo))),levels=[1.e-2, 1.e-1, 1.e0, 1.e1, 1.e2, 1.e3, 1.e4],extend='both')
axs[0].set_xscale('log')
axs[0].set_yscale('log')
cbar1 = plt.colorbar(c1,ax=axs[0])
...
axs[0].set_ylabel(r'$T$ [MeV]')
plt.subplots_adjust(top=0.9,bottom=0.1,right=0.9,left=0.1,wspace=0.12)
plt.savefig(plot_folder + sp + "/compare_antieta_"+sp+"_v4.png",dpi=300,bbox_inches='tight')
plt.close()
...
therm_leo = np.array([P, e, s, a_P, a_e, a_s]) * 1.e39 / MeV
